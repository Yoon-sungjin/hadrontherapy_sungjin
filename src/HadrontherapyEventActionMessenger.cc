...
HadrontherapyEventActionMessenger::HadrontherapyEventActionMessenger(
  HadrontherapyEventAction* EvAct)
  : eventAction(EvAct)
{
  ...
  modeCmd = new G4UIcmdWithAString("/event/calculationMode", this);
  modeCmd->SetGuidance("Select calculation mode: DoseLET or Lineal");
  modeCmd->SetParameterName("mode", false);
  modeCmd->SetCandidates("DoseLET Lineal");  // 오타 방지를 위해 후보군 지정
...
/////////////////////////////////////////////////////////////////////////////
void HadrontherapyEventActionMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  if (command == DrawCmd)
  {
  ...
  if (command == modeCmd)
  {
    eventAction->SetCalculationMode(newValue);
    if (HadrontherapyMatrix* matrix = HadrontherapyMatrix::GetInstance())
    {
      matrix->SetCalculationMode(newValue);
    }
  }
}
