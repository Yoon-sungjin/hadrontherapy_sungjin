...
HadrontherapyEventAction::HadrontherapyEventAction()
  : calculationMode("DoseLET"), drawFlag("all"), printModulo(10), pointerEventMessenger(0)
{
  hitsCollectionID = -1;
...
void HadrontherapyEventAction::EndOfEventAction(const G4Event* evt)
{
  ...
  HadrontherapyDetectorHitsCollection* CHC =
    (HadrontherapyDetectorHitsCollection*)(HCE->GetHC(hitsCollectionID));
  if (!CHC) return;

  // 공통: Hit 리스트 초기화 및 에너지 맵 생성
  matrix->ClearHitTrack();
  std::map<G4int, G4double> eventEnergyMap;

  for (size_t h = 0; h < CHC->entries(); h++)
  {
    ...
    if (edep > 0)
    {
      G4int index = matrix->Index(i, j, k);
      eventEnergyMap[index] += edep;
    }
  }

  // [핵심] 입력받은 모드에 따라 데이터 저장 방식 결정
  if (calculationMode == "Lineal")
  {
    matrix->StoreEventDataLineal(eventEnergyMap);
  }
  else if (calculationMode == "DoseLET")
  {
    matrix->StoreEventDataDose(eventEnergyMap);

    HadrontherapyLet* let = HadrontherapyLet::GetInstance();
    if (let) let->StoreLETEventData();
  }
}
